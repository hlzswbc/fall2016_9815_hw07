 ...
 */
// modification: 
// Add Future class. Add EuroDollarFuture and BondFuture subclasses
//
...
public:
  // Future ctor
  Future(string _productId, FutureType _futureType,
    date _maturityDate);
  Future();
  ...
  // Return the maturity of the future
  date GetMaturityDate() const;

  // Overload the << operator to print out the bond
  friend ostream& operator<<(ostream &output, const Future &future);
private:
  FutureType futureType;  // type of the future
  date maturityDate;      // maturity date variable
...
Future::Future(string _productId, FutureType _futureType, date _maturityDate) : Product(_productId, FUTURE), futureType(_futureType), maturityDate(_maturityDate) {}
...
ostream& operator <<(ostream &output, const Future &future) {
  output << future.ToString(future.futureType) << " " << future.maturityDate;
  return output;
}

string Future::ToString(FutureType futureType) const{
  switch (futureType) {
    case EURODOLLARFUTURE: return "Eurodollar Future";
    case BONDFUTURE: return "Bond Future";
    default: return "";
  }
}
